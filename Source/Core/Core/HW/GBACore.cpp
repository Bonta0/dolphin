      ...
      m_link_enabled(false)
{
  ...
  mLogSetDefaultLogger(&s_dummy_logger);
...
static VFile* LoadROM_Archive(const char* path)
{
  ...
  {
    VFile* vf_archive =
        VDirFindFirst(archive, [](VFile* vf_) { return mCoreIsCompatible(vf_) == mPLATFORM_GBA; });
    if (vf_archive)
    {
...
void Core::RunUntil(u64 gc_ticks)
{
  ...
  u64 d = (static_cast<u64>(end_time - start_time) * gc_frequency) + m_gc_ticks_remainder;
